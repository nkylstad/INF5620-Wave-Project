...
def solver(Lx, Ly, Nx, Ny, T, dt, c, I, q, V, f, b, version, h=0.4, exact=None, oneD=False, standing=False, make_plot=True):
    ...
    for n in range(1,N+1):
        if version == "scalar":
            u_1,u_2,E = advance_scalar(u, u_1, u_2, Nx, Ny, x, y, q, f, c1, c2, Cx2, Cy2, dt2, b, h, t[n], exact)
            if make_plot:
                if oneD:
            ...
            fv = f(X,Y,t[n])
            u_1,u_2,E = advance_vectorized(X, Y, u, u_1, u_2, q, fv, c1, c2, Cx2, Cy2, dt2, t[n], b, Nx, Ny, exact)
            if make_plot:
                if oneD:
                    plot_u(u_1, x, t[n], n, b, Lx)
                else:
    ...
    for i in range(0,Nx+1):
        for j in range(0,Ny+1):
            ...
            u[i,j] = c1*(2*u_1[i,j] - c2*u_2[i,j] + \
                ...
                dt2*f(x[i],y[j],tn))
            if exact:
                    E[i,j] = exact(x[i],y[i],tn,b,version="scalar") - u[i,j]
                   
    u_2 = u_1.copy()
    u_1 = u.copy()
    return u_1, u_2, E
    
def advance_vectorized(X, Y ,u, u_1, u_2, q, f, c1, c2, Cx2, Cy2, dt2, tn, b, Nx, Ny, exact):
