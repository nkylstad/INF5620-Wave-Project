...
def solver(Lx, Ly, Nx, Ny, T, dt, c, I, q, V, f, b, version, w=None, exact=None, oneD=False, standing=False, make_plot=True):
    ...
    for n in range(1,N):
        if version == "scalar":
            u_1, u_2, E = advance_scalar(u, u_1, u_2, Nx, Ny, x, y, q, f, c1, c2, Cx2, Cy2, dt2, b, dx, w, t[n+1], exact)
            if make_plot:
                if oneD:
            ...
            fv = f(X,Y,t[n])
            u_1, u_2, E = advance_vectorized(X, Y, u, u_1, u_2, q, fv, c1, c2, Cx2, Cy2, dt2, t[n+1], b, dx, w, Nx, Ny, exact)
            if make_plot:
                if oneD:
